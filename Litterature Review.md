...
### SVM
An other widely used model for GHI forecasting is Support Vector Machine. To elaborate on the principle of Support Vector Machines (SVM), the process involves transforming the input vector x into a high-dimensional feature space through a nonlinear mapping. Subsequently, linear regression is conducted within this feature space. In essence, SVM utilizes a nonlinear mapping technique to enhance the separability of data points, making it feasible to perform linear regression in a higher-dimensional context. This methodology enables SVM to effectively handle complex relationships and achieve superior performance in various learning tasks. These models can prevent overfitting of training data, dismissiterative tuning of model parameters, require a few kernels, make faster computations, and have good generalization and convergence.  

In Shi et al paper [12], a one-day-ahead PV power output forecasting model for a single station is derived based on the weather forecasting data, actual historical power output data, and the principle of SVM. SVM is lauded for its nonlinearity, calculating speed, and convergence results. Global variables, P1 and C, are defined, and the RBF function is selected as the kernel function. The model's efficiency is highlighted, emphasizing its dependency on day type, with better performance on sunny days. 
